...
def application_train_test(num_rows=None, nan_as_category=False):
    ...
    df = concat([df, test_df]).reset_index(drop=True)
    ...
    df.drop(['AMT_INCOME_TOTAL', 'AMT_CREDIT', 'AMT_ANNUITY'], axis=1, inplace=True)
    del test_df
    gc.collect()
    return df
...
def main(debug=False):
    ...
    with timer("Process credit card balance"):
        ...
        del cc
        gc.collect()
    with timer("Création du dataframe"):
        df.to_csv('data/interim/data_initial.csv', index=False)
